...
const CommentBox = ({ commentData }) => (
  <View style={{ flex: 1, backgroundColor: "#3a3c41", marginBottom: 2 }}>
    ...
    >
      <Image
        source={{
          uri: `https://imgur.com/user/${commentData.author_id}/avatar`,
        }}
    ...
    <Text
      style={{ marginLeft: 45, color: "white", marginBottom: 15, fontSize: 15 }}
    >
      {commentData.comment}
    </Text>
  </View>
...
export default function PhotoDetailsScreen({ route }) {
  ...
  const [faved, setFaved] = useState(data.favorite);
  const [voted, setVoted] = useState(data.vote === "up");
        ...
        <View style={{ backgroundColor: "#33353a" }}>
          <View style={styles.header}>
            <View style={styles.headerContainer}>
              <Image
                source={{
                  uri: `https://imgur.com/user/${data.account_url}/avatar`,
                }}
                style={{
                  width: 40,
                  height: 40,
          ...
          <View
            style={[styles.buttonsRow, { marginTop: data.description ? 0 : 5 }]}
          >
            <TouchableOpacity
